d3.parcoords = function (config) {
    ...
    var events = d3.dispatch.apply(this, ["render", "resize", "highlight", "brush", "brushend"].concat(d3.keys(__))),
		marginSize = function() {
			return __.vertical ? [__.margin.top-__.margin.bottom,__.margin.right-__.margin.left] : [__.margin.right-__.margin.left, __.margin.top-__.margin.bottom];
		},
		viewW = function() {
        ...
        worldW = function () {
            return __.vertical ? viewH() : viewW();
        },
        worldH = function () {
            return __.vertical ? viewW() : viewH();
		},
		viewInteriorSize = function() {
			return __.vertical ? [__.width - __.margin.right - __.margin.left, __.height - __.margin.top - __.margin.bottom]
				: [__width, __];
		},
        flags = {
            brushable: false,
    ...
    pc.autoscale = function () {
        // xscale
        xscale.rangePoints([0, worldW()], 1);

        // yscale
        __.dimensions.forEach(function (k) {
			if(__.allowNonQuantitative && __.types[k] == "string") {
				yscale[k] = d3.scale.ordinal()
					.domain(__.data.map(function(d) { return d[k]; }))
					.rangePoints([worldH() - 1, 2]);
			}
			else {
		...
		if(__.vertical) {
			for(layer in ctx) {
				ctx[layer].rotate(90*Math.PI/180);
				ctx[layer].translate(0,-worldH());			
			}
		}
    ...
    pc.detectDimensions = function () {
        ...
        var dims = __.allowNonQuantitative? d3.parcoords.nonQuantitative(__.data) : d3.parcoords.quantitative(__.data);
		pc.dimensions(_.reject(dims, function(d) { return _.contains(__.ignore, d); }));
        return this;
    };
    ...
    pc.clear = function (layer) {
        ctx[layer].clearRect(0, 0, worldW() + 2, worldH() + 2);
        return this;
    };
    ...
    pc.brushable = function () {
        ...
        g.append("svg:g")
            ...
            .attr("x", -15)
            .attr("width", 30)
			.attr("cursor",__.vertical ? "ew-resize" : "ns-resize");
	...
	pc.vertical = function() {
		__.vertical = true;
		return this;		
	};

	pc.allowNonQuantitative = function() {
    ...
    pc.reorderable = function () {
        ...
        g.style("cursor", "move")
            .call(d3.behavior.drag()
                ...
                .on("drag", function (d) {
                    dragging[d] = Math.min(viewW(), Math.max(0, this.__origin__ += d3.event.dx));
    ...
    pc.brushReset = function () {
		__.dimensions.forEach(function(dimension) {
		    yscale[dimension].brush.clear();
		});
		d3.selectAll('.brush').call(pc.brush);
        pc.autoscale();
		pc.render();
	}
    ...
    pc.resize = function () {
		...
		if(__.vertical)
	        pc.svg.attr("transform", "translate(" + (__.margin.left + worldH()) + "," + __.margin.top + ") rotate(90)");
        ...
        if (g) {
            ...
            g.selectAll("g.axis").each(function (d) {
                d3.select(this).call(axis.scale(yscale[d]));
            })
        }
    ...
    function paths(data, ctx) {
        ctx.clearRect(-1, -1, worldW() + 2, worldH() + 2);
        ctx.beginPath();
    ...
    function brush_min(p) {
        return is_brushed(p) ? yscale[p](yscale[p].brush.extent()[0]) : worldH();
    };
        ...
        return __.data
            .filter(function (d) {
                return actives.every(function (p, dimension) {
					// d: data object, d[p]: object dimension value
					if(__.types[p] == "string") {
...
d3.parcoords.nonQuantitative = function (data) {
	return d3.keys(data[0])
		.filter(function (col) {
			var v = data[0][col];
			return ((parseFloat(v) == v) || (typeof(v) == "string")) && (v != null);
		});
}
...
/*
    ...
    var types = {};
	var recordKeysMerged = {};

	debugger;

	// first figure out all the keys
	var allItems = _.keys(data);
	_.each(allItems, function(item) {
		recordKeysMerged = _.extend(recordKeysMerged, data[item]);
	 });
	
	_.each(_.pairs(recordKeysMerged), function(kv) {
		types[kv[0]] = d3.parcoords.toTypeCoerceNumbers(kv[1]);
    });
    return types;*/
};
